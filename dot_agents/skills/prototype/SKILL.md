...
4. The shared previewer should follow a carousel format for navigating between the different designs easily. Carousel should prefereably be a overline over the page in the bottom middle or in the header if necessary.
...
6. Serve the "application" so it outlives your session and give the user the url. verify that the url is accesible..
7. Give the user the names of each design and mention what else could be implemented for a suggested follow up.

The goal of this branch is to explore alternatives of a user flow for interacting with an application or a specific data pattern. 
